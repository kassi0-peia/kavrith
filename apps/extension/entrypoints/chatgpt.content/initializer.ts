...
function bootstrapMessage(initialization: InitializationPolicy): string {
  ...
  return [
    ...
    "- kavrith:patch applies structured file changes. Use it for ordinary repository edits instead of falling back to shell/Python file surgery. Update File sections use @@ hunks. Add File sections may place +content lines directly after the file header and do not require @@.",
    ...
    "- kavrith:run executes shell command text. Use it when shell features such as &&, pipes, redirects, variable expansion, or other shell syntax are required. It is self-delimiting and must end with # kavrith:end on its own final line.",
    ...
    "For kavrith:run, put the complete shell command after the directive marker and finish with # kavrith:end on its own final line. The end marker is required so a partially streamed command can never execute.",
